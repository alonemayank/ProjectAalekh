	...
	if(isset($_POST)){
		...
		if($stmt->num_rows>0){
			...
			foreach($data as $row){
				...
				echo "<tr><td><a onclick='refreshPage()' target='_blank' href='openFileOnClick.php?filename={$row['id']}&count={$row['clickCount']}&wikipedia=false'>file: {$row['file']}</a> </td>";
				echo "<td><a onclick='refreshPage()' target='_blank' href='openFileOnClick.php?filename={$row['id']}&count={$row['clickCount']}&wikipedia=true&term={$row['term']}'>Wikipedia Link</a></td></tr> ";
				echo "</table>";
		...
		if($stmt->num_rows>0){
			...
			foreach($data as $row){
				...
				echo " <tr><td>{$row['term']} </td><td>  <a onclick='refreshPage()' target='_blank' href='openFileOnClick.php?filename={$row['id']}&count={$row['clickCount']}&wikipedia=false'>file: {$row['file']}</a> </td>";
				echo "<td><a onclick='refreshPage()' target='_blank' href='openFileOnClick.php?filename={$row['id']}&count={$row['clickCount']}&wikipedia=true&term={$row['term']}'>Wikipedia Link</a></td></tr> ";
				echo "</table>";
				echo "<h4>Tags</h4><p>{$row['category']}</p>";
